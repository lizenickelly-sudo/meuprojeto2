...
function Field({ label, value, editing, onChange, placeholder, keyboard, tid, mask, locked, loading, hint }: { label: string; value: string; editing: boolean; onChange: (v: string) => void; placeholder?: string; keyboard?: 'default' | 'numeric' | 'phone-pad' | 'email-address'; tid?: string; mask?: 'cpf' | 'phone'; locked?: boolean; loading?: boolean; hint?: string }) {
  ...
  return (
    <View style={s.field}>
      ...
      {editing && !locked ? (
        <View>
          <TextInput style={s.fieldInput} value={formattedValue} onChangeText={handleChange} placeholder={placeholder} placeholderTextColor={PROFILE_THEME.textMuted} keyboardType={keyboard || 'default'} autoCapitalize={keyboard === 'email-address' ? 'none' : 'sentences'} maxLength={maxLen} testID={`in-${tid}`} />
...
const PROFILE_THEME = {
  ...
  card: 'rgba(7,11,22,0.52)',
  cardStrong: 'rgba(7,11,22,0.64)',
  cardSoft: 'rgba(255,255,255,0.06)',
  border: 'rgba(255,255,255,0.14)',
  borderSoft: 'rgba(255,255,255,0.08)',
  input: 'rgba(255,255,255,0.08)',
  inputStrong: 'rgba(255,255,255,0.1)',
};
...
export default function ProfileScreen() {
  ...
  return (
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={[s.scroll, { paddingTop: ins.top }]} keyboardShouldPersistTaps="handled">
        <View style={s.header}>
          ...
          <View style={s.headerActions}>
            {ed ? (
              ...
              <TouchableOpacity style={s.headerBtn} onPress={startEd} testID="ed-btn"><Edit3 size={20} color={PROFILE_THEME.textPrimary} /></TouchableOpacity>
            )}
            {isAdmin && <TouchableOpacity style={s.headerBtn} onPress={() => router.push('/admin')}><Settings size={20} color={PROFILE_THEME.textPrimary} /></TouchableOpacity>}
          </View>
        </View>
        ...
        <View style={s.section}>
          <TouchableOpacity style={[s.sectionHeader, openSections.personal && s.sectionHeaderExpanded]} onPress={() => toggleSection('personal')} activeOpacity={0.8}>
            ...
            <View style={[s.sectionChevronWrap, openSections.personal && s.sectionChevronOpen]}><ChevronDown size={18} color={PROFILE_THEME.textMuted} /></View>
          </TouchableOpacity>
        ...
        <View style={s.section}>
          <TouchableOpacity style={[s.sectionHeader, openSections.address && s.sectionHeaderExpanded]} onPress={() => toggleSection('address')} activeOpacity={0.8}>
            ...
            <View style={[s.sectionChevronWrap, openSections.address && s.sectionChevronOpen]}><ChevronDown size={18} color={PROFILE_THEME.textMuted} /></View>
          </TouchableOpacity>
            ...
            <View style={s.sectionBody}>
              ...
              {ed && !isFieldLocked('city') ? (
                <TouchableOpacity style={s.citySelector} onPress={() => setShowCityModal(true)}>
                  ...
                  <ChevronDown size={18} color={PROFILE_THEME.textMuted} />
                </TouchableOpacity>
        ...
        <View style={s.section}>
          <TouchableOpacity style={[s.sectionHeader, openSections.pix && s.sectionHeaderExpanded]} onPress={() => toggleSection('pix')} activeOpacity={0.8}>
            ...
            <View style={[s.sectionChevronWrap, openSections.pix && s.sectionChevronOpen]}><ChevronDown size={18} color={PROFILE_THEME.textMuted} /></View>
          </TouchableOpacity>
          {openSections.pix && (
            <View style={s.sectionBody}>
      ...
      <Modal visible={showCityModal} transparent animationType="slide" onRequestClose={() => setShowCityModal(false)}>
        <View style={cm.overlay}>
          <View style={cm.container}>
            ...
            <View style={cm.header}><Text style={cm.title}>Selecione a cidade</Text><TouchableOpacity onPress={() => { setShowCityModal(false); setCitySearch(''); }}><X size={22} color={PROFILE_THEME.textPrimary} /></TouchableOpacity></View>
            <TextInput style={cm.search} value={citySearch} onChangeText={setCitySearch} placeholder="Buscar cidade..." placeholderTextColor={PROFILE_THEME.textMuted} autoFocus />
      ...
      <Modal visible={showPinModal} transparent animationType="fade" onRequestClose={closePinModal}>
        <View style={pin.overlay}>
          <View style={pin.container}>
            <View style={pin.header}><View style={pin.lockIcon}><Lock size={24} color={Colors.dark.primary} /></View><Text style={pin.title}>Acesso Administrativo</Text><Text style={pin.subtitle}>Digite a senha de 6 dígitos</Text><TouchableOpacity style={pin.closeBtn} onPress={closePinModal}><X size={20} color={PROFILE_THEME.textMuted} /></TouchableOpacity></View>
            ...
            <View style={pin.keypad}>
              {['1','2','3','4','5','6','7','8','9','','0','del'].map((key) => {
                ...
                if (key === 'del') return <TouchableOpacity key="del" style={pin.key} onPress={handlePinDelete} activeOpacity={0.6}><Delete size={22} color={PROFILE_THEME.textPrimary} /></TouchableOpacity>;
...
const s = StyleSheet.create({
  ...
  headerTitle: { fontSize: 26, fontWeight: '800' as const, letterSpacing: 0.3, color: PROFILE_THEME.textPrimary, textShadowColor: 'rgba(0,0,0,0.32)', textShadowOffset: { width: 0, height: 2 }, textShadowRadius: 10 },
  headerActions: { flexDirection: 'row', gap: 8 },
  headerBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: PROFILE_THEME.card, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: PROFILE_THEME.border },
  savedBanner: { flexDirection: 'row', alignItems: 'center', gap: 8, marginHorizontal: 16, marginTop: 8, backgroundColor: 'rgba(16,185,129,0.12)', borderRadius: 14, padding: 12, borderWidth: 1, borderColor: 'rgba(16,185,129,0.22)' },
  savedText: { color: Colors.dark.success, fontSize: 13, fontWeight: '600' as const },
  profileHeader: { padding: 18, marginHorizontal: 16, marginTop: 12, backgroundColor: PROFILE_THEME.cardStrong, borderRadius: 24, borderWidth: 1, borderColor: PROFILE_THEME.border },
  ...
  profileName: { fontSize: 25, fontWeight: '900' as const, color: PROFILE_THEME.textPrimary, lineHeight: 28 },
  profileLocation: { fontSize: 14, color: 'rgba(241,245,249,0.92)', fontWeight: '600' as const },
  ...
  section: { marginTop: 18, marginHorizontal: 16, backgroundColor: PROFILE_THEME.cardStrong, borderRadius: 20, borderWidth: 1, borderColor: PROFILE_THEME.border, overflow: 'hidden' as const },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16, backgroundColor: PROFILE_THEME.cardSoft },
  sectionHeaderExpanded: { borderBottomWidth: 1, borderBottomColor: PROFILE_THEME.borderSoft },
  sectionHeaderMain: { flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 },
  sectionTitle: { fontSize: 16, fontWeight: '800' as const, letterSpacing: 0.2, color: PROFILE_THEME.textPrimary, flex: 1 },
  ...
  fieldLabel: { color: 'rgba(226,232,240,0.76)', fontSize: 11, fontWeight: '700' as const, textTransform: 'uppercase' as const, letterSpacing: 0.8 },
  fieldValue: { color: PROFILE_THEME.textPrimary, fontSize: 15, fontWeight: '500' as const },
  fieldValueLocked: { color: PROFILE_THEME.textSecondary },
  fieldLockedContainer: { backgroundColor: 'rgba(255,190,11,0.08)', borderRadius: 12, padding: 10, borderWidth: 1, borderColor: 'rgba(255,190,11,0.18)' },
  fieldInput: { backgroundColor: PROFILE_THEME.inputStrong, borderRadius: 12, padding: 12, color: PROFILE_THEME.textPrimary, fontSize: 15, borderWidth: 1, borderColor: PROFILE_THEME.border },
  fieldHintLoading: { color: PROFILE_THEME.textMuted, fontSize: 11, marginTop: 4, marginLeft: 4, fontStyle: 'italic' as const },
  ...
  stateChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: PROFILE_THEME.input, marginRight: 6, borderWidth: 1, borderColor: PROFILE_THEME.borderSoft },
  stateChipActive: { backgroundColor: Colors.dark.primaryFaint, borderColor: Colors.dark.primary },
  stateChipText: { color: PROFILE_THEME.textSecondary, fontSize: 13, fontWeight: '600' as const },
  stateChipTextActive: { color: Colors.dark.primary },
  citySelector: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: PROFILE_THEME.inputStrong, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: PROFILE_THEME.border },
  citySelectorText: { color: PROFILE_THEME.textPrimary, fontSize: 15 },
  citySelectorPlaceholder: { color: PROFILE_THEME.textMuted, fontSize: 15 },
  securityRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  securityRowSeparated: { marginTop: 14, paddingTop: 14, borderTopWidth: 1, borderTopColor: PROFILE_THEME.borderSoft },
  securityInfo: { flex: 1 },
  securityLabel: { color: PROFILE_THEME.textPrimary, fontSize: 15, fontWeight: '800' as const },
  securityDesc: { color: 'rgba(241,245,249,0.86)', fontSize: 13, marginTop: 4, lineHeight: 19 },
  ...
  pendingTagHint: { color: 'rgba(241,245,249,0.82)', fontSize: 11, fontWeight: '600' as const },
  ...
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, marginTop: 24, paddingVertical: 14, borderRadius: 14, backgroundColor: PROFILE_THEME.card, borderWidth: 1, borderColor: 'rgba(239,68,68,0.18)' },
  ...
  versionText: { color: PROFILE_THEME.textMuted, fontSize: 12 },
});

const cm = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' },
  container: { backgroundColor: PROFILE_THEME.cardStrong, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '75%', paddingBottom: 30, borderWidth: 1, borderColor: PROFILE_THEME.border },
  handle: { width: 40, height: 4, borderRadius: 2, backgroundColor: PROFILE_THEME.textMuted, alignSelf: 'center', marginTop: 12 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 },
  title: { fontSize: 19, fontWeight: '800' as const, color: PROFILE_THEME.textPrimary, letterSpacing: 0.2 },
  search: { marginHorizontal: 20, marginBottom: 8, backgroundColor: PROFILE_THEME.inputStrong, borderRadius: 12, padding: 12, color: PROFILE_THEME.textPrimary, fontSize: 15, borderWidth: 1, borderColor: PROFILE_THEME.border },
  list: { paddingHorizontal: 20 },
  cityItem: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: PROFILE_THEME.borderSoft },
  cityTextWrap: { flex: 1 },
  cityText: { color: PROFILE_THEME.textPrimary, fontSize: 15 },
  citySubText: { color: PROFILE_THEME.textSecondary, fontSize: 12, marginTop: 2, fontWeight: '600' as const },
  emptyText: { color: PROFILE_THEME.textMuted, fontSize: 14, textAlign: 'center' as const, paddingVertical: 24 },
});

const pin = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  container: { width: '100%', maxWidth: 340, backgroundColor: PROFILE_THEME.cardStrong, borderRadius: 24, padding: 28, borderWidth: 1, borderColor: PROFILE_THEME.border, alignItems: 'center' },
  ...
  title: { color: PROFILE_THEME.textPrimary, fontSize: 20, fontWeight: '900' as const, letterSpacing: 0.2, textAlign: 'center' as const },
  subtitle: { color: 'rgba(241,245,249,0.86)', fontSize: 13, marginTop: 6, textAlign: 'center' as const, fontWeight: '600' as const },
  closeBtn: { position: 'absolute' as const, top: 0, right: 0, width: 36, height: 36, borderRadius: 18, backgroundColor: PROFILE_THEME.cardSoft, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: PROFILE_THEME.borderSoft },
  dotsRow: { flexDirection: 'row', gap: 12, marginBottom: 8 },
  dot: { width: 16, height: 16, borderRadius: 8, borderWidth: 2, borderColor: PROFILE_THEME.textMuted, backgroundColor: 'transparent' },
  ...
  key: { width: 68, height: 56, borderRadius: 14, backgroundColor: PROFILE_THEME.inputStrong, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: PROFILE_THEME.borderSoft },
  keyText: { color: PROFILE_THEME.textPrimary, fontSize: 24, fontWeight: '700' as const },
