...
import { Wallet, ArrowUpRight, ArrowDownLeft, Star, DollarSign, Share2, Shield, Clock, X, FileText, CheckCircle, Gift, Lock, Copy, Users, Send, Ticket, Trash2, Hash, Edit3, Heart } from 'lucide-react-native';
...
export default function WalletScreen() {
  ...
  const { balance, points, transactions, profile, redeemPoints, redeemPointsPending, getPointsRedeemInfo, referralCount, getReferralCode, addTransactionRaw } = useUser();
  const {
    lotteryNumbers,
    editLotteryNumbers,
  ...
  return (
    <View style={w.ctr}>
      ...
      >
        ...
        {sharedPromotions.length > 0 && (
          <View style={w.sharesSection}>
            <View style={w.sharesHeader}><Heart size={18} color={Colors.dark.danger} /><Text style={w.sharesTitle}>Compartilhamentos</Text></View>
            <View style={w.sharesList}>
              {sharedPromotions.map((share) => (
                <View key={share.id} style={w.shareItem}>
                  <View style={w.shareInfo}>
                    <Text style={w.shareTitle}>{share.title}</Text>
                    <Text style={w.shareDate}>{getSafeDate(share.sharedAt).toLocaleDateString('pt-BR')}</Text>
                    <Text style={w.shareMeta}>{share.shares} compartilhamento{share.shares !== 1 ? 's' : ''}</Text>
                  </View>
                  <TouchableOpacity
                    style={w.shareL}
                    onPress={() => {
                      const updated = sharedPromotions.map((item) =>
                        item.id === share.id ? { ...item, likes: item.likes + 1 } : item
                      );
                      setSharedPromotions(updated);
                      if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    }}
                  >
                    <Heart size={16} fill={Colors.dark.danger} color={Colors.dark.danger} />
                    <Text style={w.shareLikeCount}>{share.likes}</Text>
                  </TouchableOpacity>
                </View>
              ))}
            </View>
...
const w = StyleSheet.create({
  ...
  sharesTitle: { fontSize: 15, fontWeight: '700' as const, color: Colors.dark.text },
  sharesList: { gap: 10 },
  ...
  shareInfo: { flex: 1 },
  shareTitle: { color: Colors.dark.text, fontSize: 13, fontWeight: '600' as const },
  shareDate: { color: Colors.dark.textMuted, fontSize: 11, marginTop: 2 },
  shareMeta: { color: Colors.dark.textSecondary, fontSize: 11, marginTop: 2 },
...
const lt = StyleSheet.create({
  ...
  numberSponsor: { color: Colors.dark.textSecondary, fontSize: 12, fontWeight: '600' as const, flex: 1 },
  numberDate: { color: Colors.dark.textMuted, fontSize: 11 },
  ...
  modalTitle: { fontSize: 18, fontWeight: '700' as const, color: Colors.dark.text },
  modalBody: { paddingHorizontal: 16, paddingVertical: 16, maxHeight: 300 },
  resultsObs: { color: Colors.dark.textSecondary, fontSize: 12, lineHeight: 18, marginBottom: 12 },
  ...
  resultContest: { color: Colors.dark.text, fontSize: 13, fontWeight: '700' as const },
  resultDate: { color: Colors.dark.textMuted, fontSize: 11 },
  resultPrizes: { gap: 6 },
  resultPrizeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  resultPrizeLabel: { color: Colors.dark.textSecondary, fontSize: 11 },
  ...
  emptyEditTxt: { color: Colors.dark.textMuted, fontSize: 13 },
  editList: { gap: 8 },
